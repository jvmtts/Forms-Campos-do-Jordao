...
function createBasinPayload(values: FormValues) {
  ...
  payload.append('_subject', `Nova inscrição — ${values.nomeCompleto} (Expedição Campos do Jordão)`)
  payload.append('Evento', 'Expedição Campos do Jordão — 21 a 23 de agosto de 2026')
...
const fieldClassName = (hasError: boolean) =>
  `h-14 w-full rounded-xl border bg-[#fdfaf6] px-4 text-[15px] text-[#33261f] outline-none transition duration-200 placeholder:text-[#ac9e95] hover:border-[#c2b4a8] focus:border-[#9a6230] focus:bg-white focus:ring-4 focus:ring-[#9a6230]/10 ${
    hasError ? 'border-red-500 bg-red-50/40' : 'border-[#ded2c5]'
  }`
...
function FieldShell({ id, label, error, required, hint, children }: FieldShellProps) {
  return (
    <div>
      <label className="mb-2.5 block text-[13px] font-semibold tracking-[0.01em] text-[#4c382d]" htmlFor={id}>
        {label}
        {required && <span className="ml-1 text-[#9a6230]">*</span>}
      </label>
      {children}
      {hint && !error && <p className="mt-2.5 text-xs leading-5 text-[#8e7f76]">{hint}</p>}
      {error && (
  ...
  return (
    <FieldShell id={id} label={label} error={error} required={required}>
      <div className="relative">
        ...
        <ChevronDown aria-hidden="true" className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#87776e]" />
      </div>
    </FieldShell>
  )
}
    ...
    <FieldShell id="estado" label="Estado" error={error} required>
      <div className="relative" ref={rootRef}>
        <button
          ...
          type="button"
        >
          <span className={value ? 'font-medium text-[#33261f]' : 'text-[#ac9e95]'}>
            {value || 'Selecione seu estado'}
          </span>
          <ChevronDown aria-hidden="true" className={`h-4 w-4 text-[#87776e] transition ${open ? 'rotate-180' : ''}`} />
        </button>

        {open && (
          <div className="absolute inset-x-0 top-[calc(100%+0.55rem)] z-30 rounded-2xl border border-[#ded2c5] bg-white p-3 shadow-[0_22px_70px_rgba(45,33,27,0.18)]" role="listbox">
            <p className="px-2 pb-2 pt-1 font-mono text-[9px] font-bold uppercase tracking-[0.18em] text-[#9b8c83]">
              Selecione a UF
            </p>
              ...
              {states.map((state) => (
                <button
                  ...
                  className={`rounded-lg px-2 py-2.5 text-sm font-semibold transition ${
                    value === state
                      ? 'bg-[#2d211b] text-white'
                      : 'text-[#614d42] hover:bg-[#f5ebdf] hover:text-[#7a4c25]'
                  }`}
                  key={state}
                  onClick={() => {
                    onChange(state)
...
function UploadField({ id, label, file, error, required = true, onChange }: UploadFieldProps) {
  ...
  return (
    ...
    >
      <label
        className={`flex min-h-28 cursor-pointer items-center gap-4 rounded-2xl border border-dashed px-5 py-4 transition hover:border-[#9a6230] hover:bg-[#fbf6ef] ${
          error ? 'border-red-500 bg-red-50' : 'border-[#d3c2b3] bg-[#fbf7f2]'
        }`}
        htmlFor={id}
      >
        <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#f2e7da] text-[#7a4c25]">
          {file ? <FileCheck2 aria-hidden="true" className="h-5 w-5" /> : <FileUp aria-hidden="true" className="h-5 w-5" />}
        </span>
        <span className="min-w-0">
          <span className="block text-sm font-semibold text-[#392a22]">
            {file ? 'Documento selecionado' : 'Selecionar documento'}
          </span>
          <span className="mt-1 block truncate text-xs text-[#87776e]">
...
function ReviewRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid gap-1 border-b border-[#e8ded4] py-3 last:border-0 sm:grid-cols-[190px_1fr] sm:gap-5">
      <dt className="text-xs font-bold uppercase tracking-[0.08em] text-[#87776e]">{label}</dt>
      <dd className="break-words text-sm font-medium text-[#392a22]">{value || 'Não informado'}</dd>
    </div>
  )
}
...
export function ExpeditionForm() {
  ...
  if (submitted) {
    return (
      <section className="bg-[#f1ece4] px-6 py-24 text-[#2d211b]" id="inscricao" ref={formTopRef}>
        <div className="mx-auto max-w-2xl rounded-3xl border border-[#dfd2c5] bg-[#fffdf9] p-8 text-center shadow-[0_24px_80px_rgba(45,33,27,0.09)] sm:p-12">
          <CheckCircle2 aria-hidden="true" className="mx-auto h-12 w-12 text-[#9a6230]" />
          <p className="mt-6 font-mono text-[11px] font-bold uppercase tracking-[0.2em] text-[#7a4c25]">Inscrição recebida</p>
          <h2 className="mt-3 font-display text-3xl font-bold">Dados enviados com sucesso.</h2>
          <p className="mx-auto mt-4 max-w-lg text-sm leading-6 text-[#78665c]">
            A equipe da Usina do Jet conferirá os dados da Expedição Campos do Jordão e entrará em contato pelo WhatsApp informado.
          </p>
        </div>
      </section>
    )
  ...
  return (
    <section className="scroll-mt-0 bg-[#f1ece4] px-4 py-16 text-[#2d211b] sm:px-8 lg:py-24" id="inscricao" ref={formTopRef}>
      <div className="mx-auto max-w-[1120px]">
        <div className="max-w-2xl sm:pl-2">
          <p className="font-mono text-[10px] font-bold uppercase tracking-[0.24em] text-[#7a4c25]">Sua inscrição</p>
          <h2 className="mt-4 font-display text-4xl font-bold leading-[1.08] tracking-[-0.04em] sm:text-5xl">Conte um pouco sobre você.</h2>
          <p className="mt-4 max-w-xl text-sm leading-6 text-[#7c6a60] sm:text-base">
            Os campos com <span className="font-bold text-[#9a6230]">*</span> são obrigatórios. Você poderá revisar tudo antes do envio.
          </p>
        </div>

        <nav aria-label="Etapas da inscrição" className="mt-10 grid grid-cols-3 overflow-hidden rounded-t-3xl border border-[#ded0c2] bg-[#fffdf9] shadow-[0_18px_55px_rgba(45,33,27,0.06)]">
          {steps.map(({ label, icon: Icon }, index) => {
            ...
            return (
              <button
                ...
                className={`flex min-h-24 items-center gap-3 border-r border-[#e6dbcf] px-3 text-left transition last:border-r-0 sm:px-6 ${
                  active ? 'bg-[#2d211b] text-white' : 'text-[#87776e]'
                ...
                type="button"
              >
                <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl ${active ? 'bg-[#9a6230] text-white' : complete ? 'bg-[#f2e7da] text-[#7a4c25]' : 'bg-[#f3ece5]'}`}>
        ...
        <form className="rounded-b-3xl border-x border-b border-[#ded0c2] bg-[#fffdf9] p-5 shadow-[0_26px_80px_rgba(45,33,27,0.09)] sm:p-9 lg:p-14" noValidate onSubmit={(event) => event.preventDefault()}>
          {step === 0 && (
            <div>
              <div className="mb-9 flex items-start gap-4 border-b border-[#ebe1d7] pb-7">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#f2e7da] font-mono text-[11px] font-bold text-[#7a4c25]">01</span>
                <div>
                  <h3 className="font-display text-2xl font-bold">Dados pessoais</h3>
                  <p className="mt-1.5 text-sm leading-6 text-[#87776e]">Use os mesmos dados que constam nos seus documentos.</p>
                </div>
              </div>
              ...
              <FieldShell id="tipoInscricao" label="Como será sua inscrição?" error={errors.tipoInscricao} required>
                <div className="mb-8 grid gap-3 sm:grid-cols-2">
                  {[
                    ...
                    <label className={`cursor-pointer rounded-xl border p-4 transition ${values.tipoInscricao === value ? 'border-[#9a6230] bg-[#f7eee3] text-[#7a4c25] ring-4 ring-[#9a6230]/5' : 'border-[#ded2c5] bg-[#fdfaf6] text-[#6f5b50] hover:border-[#9a6230]'}`} key={value}>
              ...
              {values.tipoInscricao === 'dupla' && (
                <div className="mt-10 rounded-2xl border border-[#e4d8cb] bg-[#fbf7f2] p-5 sm:p-6">
                  <p className="mb-5 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-[#7a4c25]">Dados do acompanhante</p>
                  <div className="grid gap-x-6 gap-y-7 sm:grid-cols-2">
                    <div className="sm:col-span-2">
          ...
          {step === 1 && (
            <div>
              <div className="mb-9 flex items-start gap-4 border-b border-[#ebe1d7] pb-7">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#f2e7da] font-mono text-[11px] font-bold text-[#7a4c25]">02</span>
                <div>
                  <h3 className="font-display text-2xl font-bold">Veículo e habilitação</h3>
                  <p className="mt-1.5 text-sm leading-6 text-[#87776e]">Informe como participará da experiência off-road e os dados da sua CNH.</p>
                </div>
              </div>
              ...
              <FieldShell id="modalidadeVeiculo" label="Como você participará?" error={errors.modalidadeVeiculo} required>
                <div className="grid gap-3 sm:grid-cols-2">
                  {[
                    ...
                    <label className={`cursor-pointer rounded-xl border p-4 transition ${values.modalidadeVeiculo === value ? 'border-[#9a6230] bg-[#f7eee3] text-[#7a4c25] ring-4 ring-[#9a6230]/5' : 'border-[#ded2c5] bg-[#fdfaf6] text-[#6f5b50] hover:border-[#9a6230]'}`} key={value}>
                      <span className="flex items-center gap-3 text-sm font-semibold">
              ...
              {values.modalidadeVeiculo === 'proprio' && (
                <div className="mt-6 rounded-2xl border border-[#e4d8cb] bg-[#fbf7f2] p-5 sm:p-6">
                  <p className="mb-5 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-[#7a4c25]">Seu veículo</p>
                  <FieldShell id="tipoVeiculo" label="Tipo de veículo" error={errors.tipoVeiculo} required>
                    <div className="grid gap-3 sm:grid-cols-3">
                      {([['utv', 'UTV'], ['quadriciclo', 'Quadriciclo'], ['4x4', '4x4']] as const).map(([value, label]) => (
                        <label className={`flex cursor-pointer items-center gap-3 rounded-xl border bg-white p-4 text-sm font-semibold transition ${values.tipoVeiculo === value ? 'border-[#9a6230] text-[#7a4c25] ring-4 ring-[#9a6230]/5' : 'border-[#ded2c5] text-[#6f5b50] hover:border-[#9a6230]'}`} key={value}>
              ...
              {values.modalidadeVeiculo === 'locacao' && (
                <div className="mt-6 rounded-2xl border border-[#e4d8cb] bg-[#fbf7f2] p-5 sm:p-6">
                  <p className="mb-5 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-[#7a4c25]">Preferência de locação</p>
                  <FieldShell id="preferenciaLocacao" label="Qual opção você prefere?" error={errors.preferenciaLocacao} required>
                    <div className="grid gap-3 sm:grid-cols-3">
                      {([['utv', 'UTV'], ['quadriciclo', 'Quadriciclo'], ['avaliar', 'Quero orientação']] as const).map(([value, label]) => (
                        <label className={`flex cursor-pointer items-center gap-3 rounded-xl border bg-white p-4 text-sm font-semibold transition ${values.preferenciaLocacao === value ? 'border-[#9a6230] text-[#7a4c25] ring-4 ring-[#9a6230]/5' : 'border-[#ded2c5] text-[#6f5b50] hover:border-[#9a6230]'}`} key={value}>
              ...
              <div className="mt-8">
                <label className={`flex cursor-pointer items-start gap-3 rounded-xl border p-5 ${errors.confirmacao ? 'border-red-500 bg-red-50' : 'border-[#ded2c5] bg-[#fbf7f2]'}`}>
                  <input checked={values.confirmacao} className="mt-1 h-4 w-4 accent-[#9a6230]" id="confirmacao" onChange={(event) => updateField('confirmacao', event.target.checked)} type="checkbox" />
                  <span className="text-sm leading-6 text-[#6f5b50]">
                    Confirmo que os dados e documentos informados são verdadeiros e que estou apto a participar da experiência. <strong className="text-[#9a6230]">*</strong>
                  </span>
                </label>
          ...
          {step === 2 && (
            <div>
              <div className="mb-9 flex items-start gap-4 border-b border-[#ebe1d7] pb-7">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#f2e7da] font-mono text-[11px] font-bold text-[#7a4c25]">03</span>
                <div>
                  <h3 className="font-display text-2xl font-bold">Revise antes de enviar</h3>
                  <p className="mt-1.5 text-sm leading-6 text-[#87776e]">Confira com calma. Você ainda pode voltar e corrigir qualquer informação.</p>
                </div>
              </div>

              <div className="grid gap-6 lg:grid-cols-2">
                <div className="rounded-2xl border border-[#e3d8cc] bg-[#fdfaf6] p-5 sm:p-6">
                  <div className="mb-3 flex items-center justify-between">
                    <h4 className="font-display text-lg font-bold">Dados pessoais</h4>
                    <button className="text-xs font-bold text-[#7a4c25] underline" onClick={() => setStep(0)} type="button">Editar</button>
                  </div>
                  <dl>
                ...
                <div className="rounded-2xl border border-[#e3d8cc] bg-[#fdfaf6] p-5 sm:p-6">
                  <div className="mb-3 flex items-center justify-between">
                    ...
                    <button className="text-xs font-bold text-[#7a4c25] underline" onClick={() => setStep(1)} type="button">Editar</button>
                  </div>
                  <dl>
              ...
              <div className="mt-6 flex items-start gap-3 rounded-xl border border-[#ddc6ac] bg-[#fbf6ef] p-4 text-sm leading-6 text-[#614d42]">
                <LockKeyhole aria-hidden="true" className="mt-0.5 h-5 w-5 shrink-0 text-[#7a4c25]" />
          ...
          <div className="mt-12 flex flex-col-reverse gap-3 border-t border-[#ebe1d7] pt-7 sm:flex-row sm:justify-between">
            {step > 0 ? (
              <button className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#cfc1b4] bg-white px-6 py-4 font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-[#614d42] transition hover:border-[#2d211b] hover:text-[#2d211b]" onClick={previousStep} type="button">
            ...
            {step < 2 ? (
              <button className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#2d211b] px-7 py-4 font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-white shadow-[0_12px_28px_rgba(45,33,27,0.16)] transition hover:-translate-y-0.5 hover:bg-[#9a6230]" onClick={nextStep} type="button">
                Continuar
                <ArrowRight aria-hidden="true" className="h-4 w-4" />
              </button>
            ) : (
              <button className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#9a6230] px-7 py-4 font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-white shadow-[0_12px_28px_rgba(154,98,48,0.24)] transition hover:-translate-y-0.5 hover:bg-[#74431d] disabled:cursor-wait disabled:opacity-70" disabled={submitting} onClick={submitForm} type="button">
